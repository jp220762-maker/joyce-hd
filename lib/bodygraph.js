...
const CENTERS = {
  ...
  '脾': { fill: C.sage, shape: 'poly', pts: [[300.5, 633.4], [170.5, 551.4], [170.5, 715.4]] },
...
const GATE_POS = {
  ...
  60:[450.0,758.6], 52:[484.4,758.6], 54:[397.0,784.8], 38:[397.1,819.6], 58:[397.8,853.2], 19:[503.0,784.8], 39:[502.9,819.6], 41:[502.2,853.2],
};
...
function renderBodygraph(P, D, info, name = '') {
  const pGates = new Set(Object.values(P).map(a => a.gate));
  const dGates = new Set(Object.values(D).map(a => a.gate));
  ...
  const col = (acts, x, title, color, isDesign) => {
    ...
    const numX = isDesign ? x + 62 : x - 62;
    PLANET_ORDER.forEach((p, i) => {
      const a = acts[p], y = 178 + i * 30, v = `${a.gate}.${a.line}`;
      ...
      t += isDesign
        ? `<text x="${x}" y="${y}" class="glyph" fill="${color}" text-anchor="start">${PLANET_GLYPH[p]}</text>` +
          ...
          (s ? `<text x="${numX + 8}" y="${y}" class="col fx" fill="${color}" text-anchor="start">${s}</text>` : '')
        : `<text x="${x}" y="${y}" class="glyph" fill="${color}" text-anchor="end">${PLANET_GLYPH[p]}</text>` +
          ...
          (s ? `<text x="${numX - 8}" y="${y}" class="col fx" fill="${color}" text-anchor="end">${s}</text>` : '');
    });
    return t;
  };
  ...
  const arrowsSvg = ar ? [
    arrowBox(288, 52, ar.topLeft, C.red), arrowBox(288, 90, ar.bottomLeft, C.red),
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 900 1180" font-family="Arial,'Microsoft JhengHei','微軟正黑體','PingFang TC','Noto Sans TC',sans-serif">
  <style>
    ...
    .col-h{ font-size:18px; font-weight:700; letter-spacing:1px; }
    .col  { font-size:16px; }
    .glyph{ font-size:15px; font-family:'DejaVu Sans','Segoe UI Symbol',Arial,'Microsoft JhengHei',sans-serif; }
    .fx   { font-size:11px; }
  ...
  <rect width="900" height="1180" fill="${C.bg}"/>\n  
  ${name ? `<text x="30" y="52" class="name" text-anchor="start">${name}</text>` : ''}
  ${col(D, 25, 'Design', C.red, true)}
  ${col(P, 875, 'Personality', C.black, false)}
  ${arrowsSvg}
