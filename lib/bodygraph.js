...
const C = {
  ...
  clay:      '#A98467',
  gold:      '#D9A95F',
  coffee:    '#5C4A3A',   // 摘要標題列（保留深色，維持標題列與白字對比）
  taupe:     '#A88E6E',   // 中心配色：喉嚨／根部／直覺／情緒（淺棕色，與橘色區隔）
};
...
// 九大中心配色（大地色系）：
// 頭＋G中心 → 大地黃（C.gold）／邏輯（腦＝Ajna）→ 鼠尾草綠（C.sage）
// 喉嚨＋根部＋直覺＋情緒 → 淺棕色（C.taupe）／薦骨＋意志力（心）→ 橘色（C.terracotta）
const CENTERS = {
  '頭': { fill: C.gold, shape: 'poly', pts: [[450.0, 45.0], [380.7, 128.5], [519.3, 128.5]] },
  '腦': { fill: C.sage, shape: 'poly', pts: [[450.0, 232.6], [380.7, 144.6], [519.3, 144.6]] },
  '喉': { fill: C.taupe, shape: 'rect', x: 385.0, y: 254.0, w: 130.0, h: 118.0 },
  ...
  '直覺': { fill: C.taupe, shape: 'poly', pts: [[300.5, 633.4], [170.5, 551.4], [170.5, 715.4]] },
  '情緒': { fill: C.taupe, shape: 'poly', pts: [[599.5, 633.4], [729.5, 551.4], [729.5, 715.4]] },
  '薦骨': { fill: C.terracotta, shape: 'rect', x: 385.0, y: 601.6, w: 130.0, h: 118.0 },
  '根': { fill: C.taupe, shape: 'rect', x: 385.0, y: 747.0, w: 130.0, h: 118.0 },
};
...
const GATE_POS = {
  ...
  48:[185.6,574.3], 57:[216.6,594.3], 44:[249.2,614.9], 50:[273.6,636.9], 32:[242.7,656.8], 28:[212.4,675.2], 18:[183.2,694.1], 36:[714.4,574.3],
  22:[683.4,594.3], 37:[650.8,614.9], 6:[626.4,636.9], 49:[657.3,656.8], 55:[687.6,675.2], 30:[716.8,694.1], 5:[418.3,613.2], 14:[450.0,613.2],
...
function renderBodygraph(P, D, info, name = '', opts = {}) {
  ...
  // 摘要表格:標題列咖啡色色塊 + 內容列（寬度對齊兩側閘門欄框線，比例加高，文字放大並統一置中）
  const TX = 2, TW = 896;
  const HEAD_H = 38, BODY_H = 66;
  const tableRow = (y, cells) => {
    const cw = TW / cells.length;
    let t = `<rect x="${TX}" y="${y}" width="${TW}" height="${HEAD_H}" fill="${C.coffee}"/>`;
    cells.forEach(([k], i) => {
      t += `<text x="${TX + cw*i + cw/2}" y="${y + HEAD_H/2}" class="th">${k}</text>`;
      if (i > 0) t += `<line x1="${TX + cw*i}" y1="${y+5}" x2="${TX + cw*i}" y2="${y+HEAD_H-5}" stroke="${C.bg}" stroke-width="1" opacity=".45"/>`;
    });
    t += `<rect x="${TX}" y="${y+HEAD_H}" width="${TW}" height="${BODY_H}" fill="${C.paper}" stroke="${C.line}" stroke-width="1"/>`;
    cells.forEach(([, v, en], i) => {
      const cx = TX + cw*i + cw/2;
      if (en) {
        t += `<text x="${cx}" y="${y + HEAD_H + BODY_H/2 - 12}" class="td">${v}</text>`;
        t += `<text x="${cx}" y="${y + HEAD_H + BODY_H/2 + 16}" class="te">${en}</text>`;
      } else {
        t += `<text x="${cx}" y="${y + HEAD_H + BODY_H/2}" class="td">${v}</text>`;
      }
      if (i > 0) t += `<line x1="${TX + cw*i}" y1="${y+HEAD_H+5}" x2="${TX + cw*i}" y2="${y+HEAD_H+BODY_H-5}" stroke="${C.line}" stroke-width="1"/>`;
    });
    return t;
  };
  const ROW_GAP = 14, BLOCK_H = HEAD_H + BODY_H;
  const row1Y = 902, row2Y = row1Y + BLOCK_H + ROW_GAP, row3Y = row2Y + BLOCK_H + ROW_GAP;
  const infoTable =
    tableRow(row1Y, [['類型', info.type, ''], ['人生角色', info.profile, ''], ['定義', info.definition, '']]) +
    tableRow(row2Y, [['內在權威', info.authority, ''], ['策略', info.strategy ?? '', ''], ['非自己主題', info.notSelf ?? '', '']]) +
    tableRow(row3Y, [['輪迴交叉', info.cross ?? '', '']]);

  const H = showTable ? (row3Y + BLOCK_H + 26) : 900;

  // 左右兩側閘門欄：細圓角框 + 表頭式咖啡色色塊（樣式對齊九大能量中心的邊框色/粗細）
  const gateFrame = (x1, x2, y1, y2, label) => {
    const rx = 16, barH = 44, w = x2 - x1;
    return `<rect x="${x1}" y="${y1}" width="${w}" height="${y2-y1}" rx="${rx}" fill="none" stroke="${C.line}" stroke-width="1.5"/>
      <rect x="${x1}" y="${y1}" width="${w}" height="${barH}" rx="${rx}" fill="${C.coffee}"/>
      <rect x="${x1}" y="${y1 + barH/2}" width="${w}" height="${barH/2}" fill="${C.coffee}"/>
      <text x="${(x1+x2)/2}" y="${y1 + barH/2}" font-size="16" font-weight="700" fill="${C.bg}" text-anchor="middle" dominant-baseline="central" letter-spacing="2">${label}</text>`;
  };

  const MARGIN = 24;
  const CW = 900 + MARGIN * 2;
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${CW} ${H}" font-family="'Noto Serif TC','PingFang TC','Microsoft JhengHei','微軟正黑體',serif">
  <style>
    ...
    .th   { font-size:26px; fill:${C.bg}; text-anchor:middle; dominant-baseline:central; letter-spacing:1px; font-weight:700; }
    .td   { font-size:31px; fill:${C.ink}; font-weight:700; text-anchor:middle; dominant-baseline:central; }
    .te   { font-size:17px; fill:${C.faint}; text-anchor:middle; font-weight:700; }
  </style>
  <rect width="${CW}" height="${H}" fill="${C.bg}"/>
  ...
  ${personalityOnly ? '' : gateFrame(2, 134, 104, 864, 'Design')}
  ${gateFrame(760, 898, 104, 864, 'Personality')}
  ${arrowsSvg}
