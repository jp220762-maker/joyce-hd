  ...
  try {
    ...
    if (raw) {
      ...
      return {
        ...
        chartReport: {
          ...DEFAULT_CONTENT.chartReport,
          ...(obj.chartReport || {}),
          sampleBirth: (obj.chartReport && obj.chartReport.sampleBirth) || DEFAULT_CONTENT.chartReport.sampleBirth,
          // 範例文字本身若有新版本，直接以最新預設值覆蓋資料庫裡的舊快取
          sampleText: ((obj.chartReport?.sampleTextVersion || 0) < (DEFAULT_CONTENT.chartReport.sampleTextVersion || 0))
            ? DEFAULT_CONTENT.chartReport.sampleText
            : (obj.chartReport?.sampleText ?? DEFAULT_CONTENT.chartReport.sampleText),
          sampleTextVersion: DEFAULT_CONTENT.chartReport.sampleTextVersion,
        },
        simpleRead: {
          typeDesc: { ...DEFAULT_CONTENT.simpleRead.typeDesc, ...(obj.simpleRead?.typeDesc || {}) },
          profileDesc: { ...DEFAULT_CONTENT.simpleRead.profileDesc, ...(obj.simpleRead?.profileDesc || {}) },
          definitionDesc: { ...DEFAULT_CONTENT.simpleRead.definitionDesc, ...(obj.simpleRead?.definitionDesc || {}) },
          authorityDesc: { ...DEFAULT_CONTENT.simpleRead.authorityDesc, ...(obj.simpleRead?.authorityDesc || {}) },
          centerSuffering: { ...DEFAULT_CONTENT.simpleRead.centerSuffering, ...(obj.simpleRead?.centerSuffering || {}) },
        },
        contact: {
          ...DEFAULT_CONTENT.contact,
          ...(obj.contact || {}),
          email: ((obj.contact?.emailVersion || 0) < (DEFAULT_CONTENT.contact.emailVersion || 0))
            ? DEFAULT_CONTENT.contact.email
            : (obj.contact?.email ?? DEFAULT_CONTENT.contact.email),
          emailVersion: DEFAULT_CONTENT.contact.emailVersion,
        },
      };
    }
...
function emailIndexKey(email) { return `hd:reportorders:email:${String(email).toLowerCase().trim()}`; }
...
export async function createReportOrder(order) {
  ...
  if (order.email) {
    // 用信箱建立索引，供客人日後用信箱查回自己的訂單（不依賴 email 通知信是否寄得出去）
    await kv(['LPUSH', emailIndexKey(order.email), order.orderId]);
    await kv(['EXPIRE', emailIndexKey(order.email), 60 * 60 * 24 * 30]);
  }
  return order;
}

// 補寫某筆舊訂單的信箱索引（給「用信箱查詢訂單」這個功能上線之前就已經存在的訂單用，
// 讓它們也能被查到；新訂單在 createReportOrder 就會自動寫入，不需要呼叫這個）
export async function backfillEmailIndex(orderId, email) {
  if (!hasKV()) throw new Error('資料庫尚未設定，無法補寫索引');
  if (!email) throw new Error('這筆訂單沒有 email，無法補寫索引');
  await kv(['LPUSH', emailIndexKey(email), orderId]);
  await kv(['EXPIRE', emailIndexKey(email), 60 * 60 * 24 * 30]);
}

// 依信箱查詢該信箱底下的所有訂單（最新的在前面，最多回傳 20 筆）
export async function getReportOrdersByEmail(email) {
  if (!hasKV() || !email) return [];
  const ids = await kv(['LRANGE', emailIndexKey(email), '0', '19']);
