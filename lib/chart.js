...
function transitChart(at = new Date(), name = '今日流日', showTable = false) {
  ...
  // 流日只呈現個性端（黑）：以 P 自身作為兩端輸入，圖上不會出現紅色設計啟動
  const D = P;
  const info = derive(P, D);
  return {
    P, D, info, utc: at,
    svg: renderBodygraph(P, D, info, name, { showTable, personalityOnly: true }),
    svgFull: renderBodygraph(P, D, info, name, { showTable: true, personalityOnly: true }),
  };
}
