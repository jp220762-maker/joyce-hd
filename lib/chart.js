...
// 本命盤：出生當地時間 + IANA 時區
function natalChart({ year, month, day, hour, minute, tz, name = '' }) {
  ...
  return { P, D, info, utc, svg: renderBodygraph(P, D, info, name) };
}

// 流日盤：指定時刻（預設此刻），台北時區
function transitChart(at = new Date(), name = '今日流日') {
  const t = A.MakeTime(at);
  const P = eng.allActivations(t);
  const D = eng.allActivations(eng.designTime(t));
  const info = derive(P, D);
  return { P, D, info, utc: at, svg: renderBodygraph(P, D, info, name) };
}
