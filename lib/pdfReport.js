...
export async function buildReportPdf({ title, subtitle, birthLine, sections, logoBytes, chartImageBytes, footerNote }) {
  ...
  if (chartImageBytes) {
    let chartImg = null;
    try { chartImg = await doc.embedPng(chartImageBytes); }
    catch { try { chartImg = await doc.embedJpg(chartImageBytes); } catch { chartImg = null; } }

    if (chartImg) {
      newPage();
      drawParagraph('你的人類圖', { size: 15, color: COFFEE, lineGap: 6 });
      y -= 14;

      const maxW = CONTENT_W;
      const maxH = y - MARGIN - 30;
      const ratio = Math.min(maxW / chartImg.width, maxH / chartImg.height, 1);
      const w = chartImg.width * ratio, h = chartImg.height * ratio;
      const x = MARGIN + (CONTENT_W - w) / 2;
      page.drawImage(chartImg, { x, y: y - h, width: w, height: h });
      y -= h;
      newPage();
    }
  }
